...
### three replicates and sampling interaction FC from real data.
load("/Users/koenvandenberge/Dropbox/PhD/Research/stageWiseTesting/significantFCInteractionLimma.rda")
N=30
nreps <- 2
nCst <- 2000
nT1 <- 1000
...
for(iter in 1:N){
	...
	libSize = sample(round(seq(15e6,20e6,length.out=nreps*4))) #high libSize
	nTags <- 13e3
	sampledIntFC=sample(significantFCInteractionLimma,nInt,replace=TRUE)
	
	#Fold changes: t1 unique (50%), t1 unique (50%), t1 cst (50%), t1 cst (50%), interaction
	#foldDiffT1 <- unlist(mapply(rep,c(3,1/3, 3,1/3, 2^sampledIntFC*1.75),c(nT1/2,nT1/2, nCst/2,nCst/2, nInt)))
	foldDiffT1 <- unlist(mapply(rep,c(3,1/3, 3,1/3),c(nT1/2,nT1/2, nCst/2,nCst/2)))
	foldDiffT1 <- c(foldDiffT1,2^sampledIntFC*1.75) #makes sure interaction FC is equal to sampled
	foldDiffT2 <- unlist(mapply(rep,c(3,1/3, 3,1/3, 1.75),c(nT2/2,nT2/2, nCst/2,nCst/2, nInt)))
