...
### clean up
#only keep control samples
dataClean <- dataClean[,condition=="N"]
...
#randomly split 14 control samples in 7vs7 comparison
set.seed(45)
g1=sample(1:14,7)
g2=(1:14)[-g1]
conditionMock=vector(length=14)
conditionMock[g1]="g1"
conditionMock[g2]="g2"
library(dplyr)
...
#swap expression between tx for g2 samples
dataSwap <- dataClean
dataSwap[deGeneTx$row,conditionMock=="g2"] <- dataClean[deGeneTx$rowSampled,conditionMock=="g2"]

#### DTE analysis
library(edgeR)
d=DGEList(dataSwap)
d=calcNormFactors(d)
design=model.matrix(~conditionMock)
d=estimateGLMCommonDisp(d,design)
d=estimateGLMTrendedDisp(d,design)
d=estimateGLMTagwiseDisp(d,design)
plotBCV(d)
fit=glmFit(d,design)
lrt=glmLRT(fit,coef="conditionMockg2")
...
save(performanceData,file="performanceDataOnlyControlsDTECaseStudySimulation.rda")


### FDR-TPR curve: dteResultsRealDataSimulation.pdf
par(mar=c(4,5,4,1), cex.axis=1.5)
plot(x=performanceData$fdrQval, y=performanceData$tprQval, type="l", ylab="True Positive Rate", xlab="False Discovery Proportion", ylim=c(0,1), lwd=3, bty="l", main="DTE", cex.lab=1.5, col="darkseagreen")
...
legend("bottomright",c("gene-level","transcript-level","transcript-level stage-wise"),lty=1,col=c("darkseagreen","steelblue","orange"), bty="n", cex=1.25, lwd=2)


### DTU analysis
library(DEXSeq)
sampleData2 <- data.frame(conditionMock=conditionMock)
rownames(sampleData2)=colnames(dataSwap)
dxd <- DEXSeqDataSet(countData = dataSwap,
                         sampleData = sampleData2,
                         design = ~ sample + exon + conditionMock:exon,
                         featureID = rownames(dataSwap),
                         groupID = as.character(txGeneData$gene))
dxd <- estimateSizeFactors(dxd)
dxd <- estimateDispersions(dxd, fitType="local")
dxd <- testForDEU(dxd, reducedModel = ~ sample + exon)
#save(dxd,file="dxdCaseStudySimulationOnlyControlsEvaluation.RData")
load("dxdCaseStudySimulationOnlyControlsEvaluation.RData")
plotDispEsts(dxd)
...
#save(evalDtuTxSW,file="evalDtuTxSWOnlyControls.rda")
load("evalDtuTxSWOnlyControls.rda")
tprSW=evalDtuTxSW[,1]
fprSW=evalDtuTxSW[,2]
fdrSW=evalDtuTxSW[,3]
...
#gene level
par(mar=c(4,5,4,1), cex.axis=1.5)
plot(x=evalGeneLevelSW[,"fdr"],y=evalGeneLevelSW[,"tpr"], type="n", xlab="False Discovery Proportion", ylab="True Positive Rate", ylim=c(0,1), bty="l", main="DTU", cex.lab=1.5, xlim=c(0,1))
