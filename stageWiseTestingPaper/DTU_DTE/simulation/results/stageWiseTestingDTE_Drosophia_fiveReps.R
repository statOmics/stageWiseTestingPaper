...
adjustShafferDTE2 <- function(p,alphaAdjusted){
    p <- sort(p)
    n <- length(p)
    pAdj <- vector(length=n)
    adjustments <- c(n-1) #Shaffer. if only tx, p-values is zero.
	...
	pAdj <- ifelse(p<=alphaAdjusted,0,1) #significant if lower, else not.
    }
    pAdj <- p*adjustments    
    pAdj[pAdj>1] <- 1
    # check monotone increase of adjusted p-values
...
plot(x=performanceData$fdrQval, y=performanceData$tprQval, type="l", ylab="TPR", xlab="FDR", ylim=c(0,1), lwd=2, bty="l", main="Drosophila DTE analysis", cex.lab=1.5, cex.axis=1.5)
...
legend("bottomright",c("gene-level","tx-level","tx-level stage-wise"),lty=1,col=1:3, bty="n", cex=1.5)
